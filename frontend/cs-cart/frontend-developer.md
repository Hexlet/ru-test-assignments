...
1. Список тарифных зон грузится с бэкенда асинхронно.
    ...
    Бэкенд реализовывать не нужно — список зон нужно грузить из JSON-файла *(файл `rate-areas.json` в репозиторий не попал)*.

    *(макет `picker-open.png` в репозиторий не попал)*
1. Элемент поддерживает фильтрацию по названию.
    ...
    *(макет `picker-search.png` в репозиторий не попал)*
...
4. У каждой позиции списка есть кнопка "Удалить". Ее поведение описано в п. 1.4.
    ...
    *(макет `rate-area.png` в репозиторий не попал)*
...
3. У каждой позиции списка есть кнопка "Удалить наценку", которая удаляет позицию списка.
    ...
    *(макет `rates.png` в репозиторий не попал)*
